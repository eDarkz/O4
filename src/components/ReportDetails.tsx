...
const ReportDetails: React.FC<ReportDetailsProps> = ({
  ...
  if (!report) {
    return <div className="bg-gray-800 p-6 rounded-lg shadow-lg text-white">Cargando detalles del reporte...</div>;
  }

  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-white mb-4">{report.guestName || 'Sin nombre'}</h2>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <p><strong>Habitación:</strong> {report.roomNumber || 'N/A'}</p>
        <p><strong>Reserva:</strong> {report.reservationNumber || 'N/A'}</p>
        <p><strong>Agencia:</strong> {report.agency || 'N/A'}</p>
        <p><strong>Reportado por:</strong> {report.reportedBy || 'N/A'}</p>
        <p><strong>Departamento:</strong> {report.department || 'N/A'}</p>
        <p><strong>Llegada:</strong> {report.arrivalDate ? formatDate(report.arrivalDate) : 'N/A'}</p>
        <p><strong>Salida:</strong> {report.departureDate ? formatDate(report.departureDate) : 'N/A'}</p>
        <p><strong>Estado de ánimo:</strong> {report.guestMood || 'N/A'}</p>
        <p><strong>Creado el:</strong> {report.createdAt ? formatDate(report.createdAt) : 'N/A'}</p>
      </div>
      
      <div className="mb-6">
        <h3 className="font-bold mb-2">Reporte de hechos:</h3>
        <p className="bg-gray-700 p-3 rounded">{report.incidentReport || 'Sin reporte'}</p>
      </div>
